...
class ScoringService:
    ...
    def __init__(self, artifacts_dir: str | Path='artifacts', review_threshold: float=0.3, decline_threshold: float=0.7, model_version: str='0.1.0'):
    ...
    def load(self) -> None:
        if not self.artifacts_dir.exists():
            log.warning(f'Artifacts dir {self.artifacts_dir} not found. Service starts empty - call /reload after training.')
            return
        ...
        if self.explainer is not None:
            try:
                for exp in self.explainer.explain_one(X.iloc[[0]], top_k=5):
                    reasons.append(ReasonCode(feature=exp.feature, value=float(exp.value), contribution=float(exp.contribution), direction=exp.direction))
            except Exception as exc:
                log.warning(f'Could not produce reason codes: {exc}')
        return ScoreResponse(application_id=app.id, fraud_score=round(proba, 6), decision=decision, threshold_review=self.review_threshold, threshold_decline=self.decline_threshold, reason_codes=reasons, model_version=self.model_version, scored_at=datetime.now(timezone.utc))
