...
st.set_page_config(page_title='LoanGuard | Risk Console', layout='wide', initial_sidebar_state='expanded')
...
view = st.sidebar.radio('Choose a view', ['Portfolio Overview', 'Application Triage', 'Drift Monitor'])
st.sidebar.divider()
st.sidebar.write(f"**Service**: {('ready ready' if svc.is_ready else 'not loaded not loaded')}")
st.sidebar.write(f'**Model version**: {svc.model_version}')
if view == 'Portfolio Overview':
    st.subheader('Portfolio overview')
    preds = load_test_predictions()
    if preds is None:
        st.warning('No `artifacts/test_predictions.csv` found yet. Run training first: `python scripts/train.py`')
        st.stop()
    ...
    col4.metric('Flag rate @ 0.5', f"{(preds['proba'] >= 0.5).mean():.1%}")
    st.divider()
    (c1, c2) = st.columns(2)
    with c1:
    ...
    with st.form('application_form', clear_on_submit=False):
        ...
        (col1, col2, col3) = st.columns(3)
        annual_inc = col1.number_input('Annual income ($)', 5000.0, 5000000.0, 60000.0, step=1000.0)
        ...
        purpose = col3.selectbox('Purpose', ['debt_consolidation', 'credit_card', 'home_improvement', 'other', 'major_purchase', 'small_business', 'car', 'medical', 'moving'])
        (col1, col2, col3) = st.columns(3)
    ...
    if submitted:
        ...
        app = LoanApplication(loan_amnt=loan_amnt, term=int(term), int_rate=int_rate, installment=round(installment, 2), grade=grade, sub_grade=sub_grade, emp_length=emp_length, home_ownership=home, annual_inc=annual_inc, verification_status=verif, purpose=purpose, zip_code=zip_code, addr_state=addr_state, dti=dti, delinq_2yrs=int(delinq_2yrs), revol_util=revol_util, issue_d=date.today())
        result = svc.score_one(app)
        (c1, c2, c3) = st.columns([1, 1, 2])
        c1.metric('Fraud score', f'{result.fraud_score:.3f}')
        c2.metric('Decision', result.decision, delta_color='off' if result.decision == 'APPROVE' else 'inverse')
        c3.markdown(f'**Thresholds** - review >= {result.threshold_review:.2f}, decline >= {result.threshold_decline:.2f}')
        if result.reason_codes:
            ...
            fig = px.bar(df_reasons, x='contribution', y='feature', orientation='h', color='contribution', color_continuous_scale='RdBu_r')
            st.plotly_chart(fig, use_container_width=True)
    ...
    if not monitor_path.exists():
        st.info('Drift monitor not yet built. Run training, then a fit() of `DriftMonitor` is included in the artifacts.')
        st.stop()
    ...
    if uploaded is not None:
        ...
        st.markdown(f"**Features in alert (PSI > {monitor.psi_alert}):** {report['alert'].sum()}")
