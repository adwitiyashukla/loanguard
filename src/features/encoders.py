...
class WoEEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'WoEEncoder':
        ...
        for col in X.columns:
            ...
            grouped['iv_part'] = (grouped['good'] / total_good - grouped['bad'] / total_bad) * grouped['woe']
...
class TargetEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'TargetEncoder':
        ...
        for col in X.columns:
            ...
            smoothed = (agg['mean'] * agg['count'] + self.global_mean_ * self.smoothing) / (agg['count'] + self.smoothing)
