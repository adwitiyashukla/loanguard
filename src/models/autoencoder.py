...
class AutoencoderFraudModel(FraudModel):
    name = 'autoencoder'
    DEFAULT_PARAMS = {'encoder_dims': [128, 64, 32], 'dropout': 0.2, 'batch_size': 512, 'epochs': 30, 'learning_rate': 0.001, 'early_stopping_patience': 5, 'noise_std': 0.05}
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None=None, **kwargs) -> 'AutoencoderFraudModel':
        if y is not None:
            mask = y.values == 0
            X_train = X.loc[mask]
            log.info(f'[AE] training on {mask.sum():,} negative samples (skipping {(~mask).sum():,} positives)')
        else:
            X_train = X
        X_arr = self.scaler.fit_transform(X_train.values).astype(np.float32)
        input_dim = X_arr.shape[1]
        self.net = _AENet(input_dim=input_dim, encoder_dims=self.params['encoder_dims'], dropout=self.params['dropout']).to(self.device)
        ...
        self._trained = True
        log.info(f'[AE] trained. recon_err range [{self.score_min_:.4f}, {self.score_max_:.4f}]')
        return self
