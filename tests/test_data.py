...
def test_synthetic_loader_shape():
    ...
    assert df['loan_amnt'].between(500, 50000).all()
...
def test_fraud_labels_columns_present(synthetic_df):
    for col in ('rule_fpd', 'rule_income_anomaly', 'rule_debt_inconsist', 'rule_address_ring', 'n_anomalies', 'is_fraud'):
...
def test_label_config_from_dict():
    cfg = LabelConfig.from_dict({'rules': {'first_payment_default': {'enabled': False, 'max_days_to_default': 60}}})
