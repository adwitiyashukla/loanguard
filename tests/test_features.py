...
import pandas as pd
import pytest
from src.features import FeatureBuilder, WoEEncoder, TargetEncoder, build_behavioral_features, build_velocity_features

def test_behavioral_features_add_columns(small_df):
    out = build_behavioral_features(small_df)
    for col in ('loan_to_income', 'credit_history_years', 'n_negative_events', 'flag_high_dti', 'app_month'):
...
def test_feature_builder_end_to_end(small_df):
    y = small_df['is_fraud']
    X = small_df.drop(columns=['is_fraud', 'rule_fpd', 'rule_income_anomaly', 'rule_debt_inconsist', 'rule_address_ring', 'n_anomalies', 'loan_status', 'last_pymnt_d'])
