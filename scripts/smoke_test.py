...
def main() -> None:
    ...
    drop = ['is_fraud', 'rule_fpd', 'rule_income_anomaly', 'rule_debt_inconsist', 'rule_address_ring', 'n_anomalies', 'loan_status', 'last_pymnt_d', 'id']
    ...
    try:
        xgb = XGBFraudModel(params={'n_estimators': 50, 'max_depth': 4}).fit(X_train, y_train, eval_set=[(X_val, y_val)])
        lgb = LGBMFraudModel(params={'n_estimators': 50, 'num_leaves': 15}).fit(X_train, y_train, eval_set=[(X_val, y_val)])
    ...
    try:
        ens = StackingFraudEnsemble(base_models=[xgb, lgb, iso], n_folds=3, calibration='isotonic').fit(X_train, y_train, X_val=X_val, y_val=y_val)
        proba = ens.predict_proba(X_test)
        metrics = binary_classification_metrics(y_test, proba)
        print(f"   ensemble: AUC={metrics['roc_auc']:.3f}, PR-AUC={metrics['pr_auc']:.3f}, KS={metrics['ks']:.3f}")
    ...
    try:
        ...
        print(f"   optimal threshold = {best['threshold']:.2f}, cost/applicant = ${best['cost_per_applicant']:.2f}")
    except Exception as e:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert np.allclose(Xb1.values, Xb2.values, atol=1e-06), 'Re-loaded FeatureBuilder produces different features'
            p1 = ens.predict_proba(X_test)
            p2 = ens2.predict_proba(X_test)
            assert np.allclose(p1, p2, atol=1e-06), 'Re-loaded model produces different scores'
            print('   serialization roundtrip OK')
            step('9. ScoringService end-to-end')
            svc = ScoringService(artifacts_dir=tmp_path)
            svc.load()
            assert svc.is_ready
            app = LoanApplication(loan_amnt=15000, term=36, int_rate=13.5, installment=509.0, grade='B', sub_grade='B3', emp_length=4.0, home_ownership='RENT', annual_inc=60000, verification_status='Verified', purpose='debt_consolidation', zip_code='941xx', addr_state='CA', dti=18.0, revol_util=45.0, revol_bal=5000, open_acc=8, total_acc=15, issue_d=date.today())
    ...
    print('\nOK ALL SMOKE CHECKS PASSED - safe to commit.\n')
